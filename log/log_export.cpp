...
#pragma comment(lib, "version")
	...
	void getOSInfo()
	{
		...
		LogTrace("[OS]操作系统: %s, 版本：%d.%d, 计算机名：%s，用户名：%s.", os_name.c_str(),
	...
	void getCPUInfo()
	{
		...
		LogTrace("[CPU]CPU型号：%s%s%s.", cpu_manufacture,
			cpu_type,
			cpu_freq);
	}
	...
	void getMemoryInfo()
	{
		...
		LogTrace("[Memory]内存共%.2fGB，当前可用%.2fGB.",
	...
	void getGPUInfo()
	{
		...
		for (size_t i = 0; i < vAdapters.size(); i++)
		{
			...
			LogTrace("[VideoCard]显卡%d，专用视频内存：%.2fMB，系统视频内存：%.2fMB，设备描述：%s.",
				i + 1,
	...
	void getHardDiskInfo()
	{
		...
		LogTrace("[HardDisk]HardDisk Serial Number: %s",
			hd_seiral.c_str());
	}
...
void LogOutputSystemMessage(HMODULE module)
{
	LogTrace("**********************永不宕机      永无BUG**********************");
	LogTrace("┌─────────────────────────────────────────────────────────────┐");
	LogTrace("│┌───┬───┬───┬───┬───┬───┬───┬───┬───┬───┬───┬───┬───┬───┬───┐│");
	...
	Hardware_Information::getFileInfo(module);
}


LOG_API void SetLogFileName(const char* log_name, HMODULE module, bool console)
{
	Log::GetInstance().InitLog(log_name, console);
	LogOutputSystemMessage(module);
}
...
LOG_API void LogTrace(const char* format, ...)
{
	if (Log::GetInstance().GetLogger()->level() <= spdlog::level::trace)
	{
		...
		Log::GetInstance().GetLogger()->debug(buffer.data());
	}
}
...
LOG_API void LogInfo(const char* format, ...)
{
	if (Log::GetInstance().GetLogger()->level() <= spdlog::level::info)
	{
		...
		Log::GetInstance().GetLogger()->info(buffer.data());
	}
}
...
LOG_API void LogWarn(const char* format, ...)
{
	if (Log::GetInstance().GetLogger()->level() <= spdlog::level::warn)
	{
		...
		Log::GetInstance().GetLogger()->warn(buffer.data());
	}
}
...
LOG_API void LogError(const char* format, ...)
{
	if (Log::GetInstance().GetLogger()->level() <= spdlog::level::err)
	{
		...
		Log::GetInstance().GetLogger()->error(buffer.data());
	}
}
...
LOG_API void LogCritical(const char* format, ...)
{
	if (Log::GetInstance().GetLogger()->level() <= spdlog::level::critical)
	{
		...
		Log::GetInstance().GetLogger()->critical(buffer.data());
	}
}
